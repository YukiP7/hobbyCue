    ...
    <div className="container mx-auto px-4 py-16 ">
      <div className="flex gap-8 m-4 w-full  flex-wrap justify-center items-center ">
        {features.map((feature, index) => (
          <div
            key={index}
            className={` w-2/5 p-4 border rounded-xl transition-all duration-300 ${feature.hoverColor} hover:text-white cursor-pointer  h-56`}
          >
            <h3 className="text-xl font-bold mb-4">{feature.title}</h3>
            <p className="">{feature.description}</p>
            <button className='text-purple-700 border border-purple-700 p-2 h-10 w-32 rounded-lg m-4'>{feature.buttonName}</button>
          </div>
        ))}
      </div>
    </div>
